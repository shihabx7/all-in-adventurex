...
import { aboutPageQuery } from "../../lib/query/singlePageQury";
import {
  getSinglePageMeta,
  getSinglePageData,
  getTeamMemberList,
...
export const getAboutUsPageData = async () => {
  ...
  const pegeRes = await fetch(aboutPageQuery, apiSetting);
  ...
  const data = {
    ...
    pageMeta: getSinglePageMeta(seoData, ftImage, "about"),

    pageData: getSinglePageData(pageResData, totalLocations),
    teamMembersList: getTeamMemberList(pageResData.teamMemberList),
    partnerMediaList: getPartnerMediaList(pageResData.partnerMediaList),
  };
  return data;
};

/*const  oldTeamMembersList= [
  {
    id: 1,
    name: "John Reichel",
