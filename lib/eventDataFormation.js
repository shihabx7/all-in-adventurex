...
const getSocialMeta = (metaObj, ftImg, activityName) => {
  let retObj = {
    title: false,
    description: false,
    hashTags: false,
...
const getListSocialMeta = (metaObj, ftImg) => {
  let retObj = {
    title: false,
    description: false,
    hashTags: false,
...
const getEventList = (listArr) => {
  ...
  for (let i = 0; i < listArr.length; i++) {
    ...
    if (eventDatav2[listArr[i].attributes.eventSlug]) {
      let obj = {
        id: i + 1,
...
export const getListPageData = (
  ...
  let pageData = {
    ...
    guestCapacity:
      pageInfo.guestCapacity != null ? pageInfo.guestCapacity : "1M+",
