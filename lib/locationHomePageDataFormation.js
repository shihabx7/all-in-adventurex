...
const getSocialMeta = (metaObj, ftImg) => {
  ...
  if (metaObj !== null) {
    ...
    retObj.hashTags = metaObj.hashTags !== null ? metaObj.hashTags : false;
    retObj.imageUrl =
  ...
  for (let i = 0; i < listArr.length; i++) {
    if (listArr[i].activity.data.attributes.activityInfo.category == cat) {
      let obj = {
        ...
        price:
          listArr[i].activityPrice.length > 0
            ? listArr[i].activityPrice
            : listArr[i].activity.data.attributes.activityInfo.price,
        description: listArr[i].pageSubTitle
          ? listArr[i].pageSubTitle
  ...
  for (let i = 0; i < listArr.length; i++) {
    if (listArr[i].activity.data.attributes.activityInfo.category == cat) {
      let obj = {
        ...
        price: listArr[i].activityPrice
          ? listArr[i].activityPrice
          : listArr[i].activity.data.attributes.activityInfo.price,
        fromPrice:
          listArr[i].activityPrice.length > 0 &&
          listArr[i].activityPrice.length < 2
            ? listArr[i].activityPrice[0].Price
            : listArr[i].activity.data.attributes.activityInfo.price[0].Price,
        description: listArr[i].pageSubTitle
          ? listArr[i].pageSubTitle
          : listArr[i].activity.data.attributes.activitySlider.sliderText,
        carouselImage: getImageData(
...
export const getLocEscapeGameCarouselSectionData = (
  listArr,
  bookingInfo,
  sectionData
) => {
  ...
  let retObj = {
    sectionTitle: sectionData.sectionTitle
      ...
      : "STEP INTO THE ACTION — OUR IN-PERSON ESCAPE ROOMS",
    sectionSubTitle: sectionData.sectionSubTitle
      ? sectionData.sectionSubTitle
      : "Ready for the real deal? Dive into thrilling, hands-on escape room adventures packed with detailed props, clever puzzles, and unforgettable moments—perfect for families, friends, birthdays, and team-building fun!",
    gameList: actList,
  };
  return retObj;
};
