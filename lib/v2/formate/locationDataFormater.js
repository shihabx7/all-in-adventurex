    ...
    .map((state) => {
      ...
      const filteredLocations = state.locations
        .map((location) => {
          ...
          const matchingActivity = location.locationActivities.find(
            (activity) => {
              return (
...
export function filterLocationListForParty(originalStateList, targetPartyName) {
  const isActiveStatus = true;
  const filteredStates = originalStateList
    .map((state) => {
      // 1. Filter the locations within the current state
      const filteredLocations = state.locations
        .map((location) => {
          // Find the matching activity within this location
          const matchingActivity = location.escapeGameParty.find((party) => {
            return (
              party.partyName.trim().toLowerCase() ===
              targetPartyName.trim().toLowerCase()
            );
          });
          ...
          if (matchingActivity) {
            ...
            return {
              ...
              bookingInfo: {
                ...
                // Add the new property exactly as requested
                itemNo: matchingActivity.bookingItemNo,
                itemFlow: matchingActivity.bookingFlow,
              },
            };
          } else {
            // If no match found, this location is excluded from the new state's locations array
