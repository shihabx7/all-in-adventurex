...
const getNewActivityPageData = (gameSlug) => {
  let escapeGameObj = escapeGameRootPageData.find(
    (obj) => obj.slug === gameSlug,
  );
  if (escapeGameObj) {
    return escapeGameObj;
  }
  return OtherGameListData.find((obj) => obj.slug === gameSlug);
};

export const activityPageDataFormaterV2 = {
  pageHeroData: function (data) {
    let newData = getNewActivityPageData(data.activitySlug);
    return {
      pageTitle: data.pageHeroData.pageTitle
        ? data.pageHeroData.pageTitle
        : data.activityName + " ESCAPE ROOM",
      pageSubTitle: data.pageHeroData.pageSubTitle
        ? data.pageHeroData.pageSubTitle
        : newData.pageHeroData.pageSubTitle,
      heroBgImageDesktop: newData.pageHeroData.heroBgImageDesktop
        ? newData.pageHeroData.heroBgImageDesktop
        : getImageData(data.pageHeroData.pageHeroDesktop),
      heroBgImageMobile: newData.pageHeroData.heroBgImageMobile
        ? newData.pageHeroData.heroBgImageMobile
        : getImageData(data.pageHeroData.pageHeroMobile),
  ...
  gameStatData: function (data) {
    ...
    if (data.locActivityData) {
      let stObj = {
        ...
        price:
          data.activityInfo.price.length > 0
            ? data.activityInfo.price[data.activityInfo.price.length - 1].Price
            : newData.gameInfo.price, // price data arry "26.99",
        gameStateBgImage: newData.gameCardData.gameStateBgImage
          ? newData.gameCardData.gameStateBgImage
          : {
  ...
  storyLineSectionData: function (data) {
    ...
    let retObj = {
      sectionTitle:
        data?.storyLine?.storyLineLabel || newData.gameInfo.storyLineTitle,
      storyLineTextTitle: newData.gameInfo.storyLineTextTitle
        ? newData.gameInfo.storyLineTextTitle
        : "Breakout or Bust!",
      storyLineText:
        data?.storyLine?.storyLineText || newData.gameInfo.storyLine,

      gameTheme: newData.gameInfo.gameTheme,
      recommendFor: newData.gameInfo.recommendFor,
      
      gamePoster: newData.gameCardData.poster,
      gameVideo: newData.gameCardData.video,
    };
    return retObj;
  },
  ...
  mooreEscapeGameCarouselSectionData: function (data) {
  let newData = getNewActivityPageData(data.activitySlug);
...
export const activityListPageDataFormeterv2 = {
  pageHeroData: function (data) {
