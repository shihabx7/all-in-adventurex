import { partyPackageListV2 } from "./v2/data/eventDataV2";
import { eventDatav2 } from "./v2/data/eventDataV2";
...
const eventList = (listArr) => {
  ...
  for (let i = 0; i < listArr.length; i++) {
    //   let newEv=eventDatav2[listArr[i].attributes.eventSlug]? true:false
    if (eventDatav2[listArr[i].event.data.attributes.eventSlug]) {
      let obj = {
        id: i + 1,
...
export const getPageMeta = (
  ...
  let meataData = {
    ...
    url: siteUrl + "/" + locationSlug + "/events/" + locEventData.eventSlug,
    ...
    structuredData:
      locEventData.seo.structuredData != null
        ...
        : false,
    canonicalURL:
      locEventData.seo.canonicalURL !== null
        ? locEventData.seo.canonicalURL
        : siteUrl + "/" + locationSlug + "/events/" + locEventData.eventSlug,
    metaindex: true,
    ...
    keywords:
      locationListSeo.keywords !== null
        ...
        : "escape rooms party and events in " +
          ...
          ",escape rooms in " +
          locationName,
    url: siteUrl + "/" + locationSlug + "/events",
    metaRobot: seoData.metaRobots !== null ? seoData.metaRobots : "all",
    structuredData:
      seoData.structuredData !== null ? seoData.structuredData : false,
    canonicalURL:
      locationListSeo.canonicalURL !== null
        ? locationListSeo.canonicalURL
        : siteUrl + "/" + locationSlug + "/events",
    metaindex: true,
    metaimg: mediaURL + seoData.metaImage.data.attributes.url,
...
//party packages
const getPartyCardImageV2 = (reqPartyName) => {
  let pname = reqPartyName.trim().toLowerCase();
  let retPartyCarImageObj = null;
  for (let i = 0; i < partyPackageListV2.length; i++) {
    let listPname = partyPackageListV2[i].partyName.trim().toLowerCase();
    if (pname == listPname) {
      retPartyCarImageObj = partyPackageListV2[i].partyCardImage;
    }
  }
  return retPartyCarImageObj;
};
  ...
  if (escapeGameParty.length > 0) {
    for (let i = 0; i < escapeGameParty.length; i++) {
      let retObj = {
        ...
        partyCardImage: getPartyCardImageV2(escapeGameParty[i].partyName),
