    ...
    >
      ...
      <div className="section-container max-w-7xl mx-auto relative z-30 pb-16 md:pb-20 lg:pb-28 px-4">
        <div className="section-title">
          <TitleSeparator
            title="BROWSE OUR OPEN POSITIONS"
            color="dark-gold"
            weight="font-bold"
          />
          <p className="max-w-[800px] md:text-lg mx-auto text-center mt-4 md:mt-8 mb-10 md:mb-12 lg:mb-16 text-gray-700">
        ...
        <div className="core-box-open-pos grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 lg:gap-10">
          {props.jobs.map((job) => {
            return (
              <div
                key={job.id}
                className="core-card rounded-xl bg-[#FDF0D1] p-6 lg:p-8"
              >
                <h3 className="text-[#111111] font-semibold text-xl md:text-2xl lg:text-3xl ">
                  {job.job_title}
                </h3>
                <p className="md:text[18px] lg:text-[20px] text-[#333333] mt-2 md:mt-4">
                  {job.short_desc}
                </p>
                <div className="jop-pg-link mt-6 lg:mt-8">
                  <a
                    href={"/careers/" + job.job_slug}
